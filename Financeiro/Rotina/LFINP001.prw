...
/*/ Rotina LFINP001
    ...
    Importação do XML de CTE, geração dos registros 
...
User Function LFINP001(aParam)
  ...
  Local aNomeArq  := {}
   ...
   If IsBlind()      // Preparação do ambiente em caso de start via Job.
      RpcClearEnv()
      RpcSetType(3) 
   ...
   ConOut("/*-------------------------------------------------------")
   ConOut("   Iniciou o processo de Importação do XML (CTE)")
   ConOut("   ---------------------------------------------")
